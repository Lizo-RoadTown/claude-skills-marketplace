...
# Registry endpoint. Same shape as session_start.py's _AGENT_CONTEXT_DEFAULT_URL.
_REGISTRY_DEFAULT_URL = "https://loom-architecture-registry.onrender.com"
...
def _registry_url() -> str:
    """The registry base URL. Allows override via LOOM_ARCHITECTURE_REGISTRY_URL
    for testing or future hosted-multitenant deploys."""
    url = os.environ.get("LOOM_ARCHITECTURE_REGISTRY_URL") or _REGISTRY_DEFAULT_URL
...
def post_candidate(
    *,
    ...
    return _http_json("POST", _registry_url() + _CANDIDATES_PATH, body)
...
def patch_candidate_status(
    ...
    return _http_json("PATCH", url, body)
...
def scan_and_emit(
    *,
    ...
    project_intelligence_dir = cwd / ".project-intelligence"
    if not project_intelligence_dir.exists():
